...
async function getOrder(id: string) {
  ...
  const { data, error } = await supabase
    .from('orders')
    .select('*, items:order_items(*, product:products(*, category:categories(*), brand:brands(*)))')
    .eq('id', id)
    .single()
...
export default async function OrderPage({ 
  ...
  const isCancelledPayment = paymentStatus === 'cancelled'
        ...
        {isSuccessfulPayment && (
          <div className="mb-6 bg-green-900/20 border border-green-500/30 rounded-lg p-4 flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-green-300 text-sm">Your order has been confirmed and is being processed.</p>
            </div>
          </div>
        )}
        {isCancelledPayment && (
          <div className="mb-6 bg-yellow-900/20 border border-yellow-500/30 rounded-lg p-4 flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-yellow-300 text-sm">Your order is pending. You can complete payment later from your profile.</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-gray-900 rounded-xl shadow-sm border border-white/10 p-6 md:p-8">
          ...
          <div className="flex items-center justify-between mb-6 pb-6 border-b border-white/10">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-3xl font-bold text-white">{getStatusText(order.status)}</h1>
                <p className="text-gray-400">Order #{order.id.slice(0, 8).toUpperCase()}</p>
              </div>
            </div>
            <Link
              href="/profile"
          ...
          <div className="mb-8 bg-gray-800/50 rounded-lg p-6 border border-white/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h3 className="font-semibold text-white mb-3 text-sm uppercase tracking-wide">Order Information</h3>
                <div className="text-gray-300 space-y-2 text-sm">
                  ...
                  <p>
                    ...
                    <span className={`capitalize font-semibold ${
                      order.status === 'processing' || order.status === 'delivered' ? 'text-green-400' :
                  ...
                  {order.payment_reference && (
                    <p>
                      <span className="font-medium text-gray-400">Payment Reference:</span>{' '}
                      <span className="font-mono">{order.payment_reference}</span>
                    </p>
                  )}
                  {order.paid_at && (
                    <p>
                      <span className="font-medium text-gray-400">Paid At:</span>{' '}
              ...
              <div>
                <h3 className="font-semibold text-white mb-3 text-sm uppercase tracking-wide">Customer Information</h3>
                <div className="text-gray-300 space-y-2 text-sm">
                  <p>
          ...
          {/* Shipping Address */}
          <div className="mb-8">
            <h2 className="font-semibold text-white mb-4 text-lg">Shipping Address</h2>
            <div className="bg-gray-800/50 rounded-lg p-4 border border-white/5">
              <div className="text-gray-300 space-y-1">
                <p className="font-medium text-white">{order.shipping_address?.name || 'N/A'}</p>
          ...
          <div className="border-t border-white/10 pt-6">
            <h2 className="font-semibold text-white mb-4 text-lg">Order Items</h2>
              ...
              {items.map((item: any) => (
                ...
                >
                  <div className="flex-1">
                    <p className="font-medium text-white mb-1">{item.product?.name || 'Product'}</p>
