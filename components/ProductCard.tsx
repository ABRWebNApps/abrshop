...
export default function ProductCard({ product, showNewBadge = false }: ProductCardProps) {
  ...
  const handleLike = (e: React.MouseEvent) => {
    ...
    let favourites: string[] = savedFavourites ? JSON.parse(savedFavourites) : []
    ...
    <Link href={`/products/${product.id}`}>
      ...
      >
        ...
        <div className="p-3 md:p-4">
          <p className="text-[10px] md:text-xs text-gray-400 mb-1 uppercase tracking-wide line-clamp-1">{categoryName}</p>
          <h3 className="font-semibold text-white mb-1 md:mb-2 line-clamp-2 text-xs md:text-sm">{product.name}</h3>
          <p className="text-xs md:text-sm text-gray-400 font-medium">{formatCurrency(product.price)}</p>
        </div>
      </div>
    </Link>
  )
